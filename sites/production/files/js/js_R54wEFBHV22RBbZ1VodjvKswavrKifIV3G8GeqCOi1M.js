...
Drupal.facetapi.applyLimit = function(settings) {
 if (settings.limit > 0 && !$('ul#' + settings.id).hasClass('facetapi-processed')) {
   var $facet = $('ul#' + settings.id);

   // Only process this code once per page load.
   $facet.addClass('facetapi-processed');

   // Ensures our limit is zero-based, hides facets over the limit.
   var limit = settings.limit - 1;
   $facet.find('li:gt(' + limit + ')').hide();
   ...
   $facet.filter(function() {
     ...
     $('<a href="#" class="facetapi-limit-link"></a>').text(Drupal.t(settings.showMoreText)).click(function() {
       ...
       if ($list.find('li:hidden').length > 0) {
         $list.find('li:gt(' + limit + ')').slideDown();
         $link.addClass('open').text(Drupal.t(settings.showFewerText));
       }
       else {
         $list.find('li:gt(' + limit + ')').slideUp();
         $link.removeClass('open').text(Drupal.t(settings.showMoreText));
       }
       return false;
     }).insertAfter($(this));
   });
